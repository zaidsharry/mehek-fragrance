...
export const Route = createFileRoute("/contact")({
  ...
  head: () => ({
    meta: [
      { title: "Contact — Mehek Fragrances" },
      { name: "description", content: "Speak with a Mehek Fragrances concierge. WhatsApp +91 82913 88799." },
      { property: "og:title", content: "Contact — Mehek Fragrances" },
      { property: "og:description", content: "Our concierge is always on WhatsApp." },
  ...
  return (
    <div className="pt-32">
      <div className="mx-auto grid max-w-[1440px] gap-16 px-6 md:grid-cols-2 md:px-10">
        ...
        >
          <div className="label-eyebrow">The Concierge</div>
          <h1 className="mt-6 font-display text-5xl leading-[0.95] md:text-7xl">
            A private line<br />
            <span className="italic text-gold-gradient">into the atelier</span>.
          </h1>
          <p className="mt-8 max-w-md font-display text-lg text-foreground/80">
            Fragrance guidance, gifting advice, custom coffrets — whisper to us and we'll answer, personally.
          </p>
          ...
          <div className="mt-12 space-y-6">
            ...
            <ContactRow icon={<Mail className="h-5 w-5" />} label="Email" value="concierge@mehekfragrances.com" href="mailto:concierge@mehekfragrances.com" />
            <ContactRow icon={<MapPin className="h-5 w-5" />} label="Atelier" value="Mumbai, India" />
          </div>
        </motion.div>
        ...
        >
          <img src={heroBottle} alt="Mehek atelier" className="h-full w-full object-cover" />
          <div className="pointer-events-none absolute inset-4 border border-gold/30" />
          <div className="pointer-events-none absolute inset-x-0 bottom-0 h-40 bg-gradient-to-t from-ink to-transparent" />
          <div className="absolute inset-x-0 bottom-0 p-8">
            <div className="label-eyebrow">Est. 2026</div>
            <div className="mt-2 font-display text-3xl text-foreground">Mehek Atelier, Mumbai</div>
          </div>
        </motion.div>
      </div>
      ...
      <section className="mx-auto mt-32 max-w-3xl px-6 md:px-10">
        <div className="text-center">
          <div className="label-eyebrow">Or send a note</div>
          <h2 className="mt-4 font-display text-4xl md:text-5xl">Write to the maison</h2>
        </div>
        <form onSubmit={(e) => e.preventDefault()} className="mt-10 space-y-6">
          ...
          <div className="text-center">
            <button className="btn-gold btn-gold-hover">Send whisper</button>
          </div>
        </form>
      </section>
    </div>
...
function ContactRow({
  ...
  const inner = (
    <div className={`glass-luxe group flex items-center gap-5 p-5 transition-all duration-500 ${href ? "hover:border-gold/50 hover:translate-x-1" : ""}`}>
      <div className={`flex h-11 w-11 items-center justify-center rounded-full ${primary ? "text-ink" : "text-gold"}`} style={primary ? { background: "var(--gradient-gold)" } : { border: "1px solid oklch(0.82 0.13 82 / 0.4)" }}>
        {icon}
      </div>
      <div>
